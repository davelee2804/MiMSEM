...
class AdvEqn:
	def __init__(self,topo,quad,dX,dt,vel):
		...
		self.La = self.M1 + 0.25*dt*(self.A + self.At)
		self.Ra = self.M1 - 0.25*dt*(self.A + self.At)
		self.La_inv = la.inv(self.La)
		self.S2 = self.La_inv*self.Ra
		...
		#L = M1_c_c + 0.25*dt*(A_dep + A_up2) # energy conserving
		L_inv = la.inv(L);
		...
		#R = M1_c_c - 0.25*dt*(A_dep + A_up2) # energy conserving
		self.Q_up = L_inv * R
