...
plt.plot((1.0*kc[:,0])/(0.5*ne*N),ui[0]*np.abs(wc)/(0.5*ne*N),'go')
plt.plot((1.0*kd[:,0])/(0.5*ne*N),ui[0]*np.abs(wd)/(0.5*ne*N),'bo')
plt.plot((1.0*ku[:,0])/(0.5*ne*N),ui[0]*np.abs(wu)/(0.5*ne*N),'r+')
plt.plot((1.0*kc[:,0])/(0.5*ne*N),(1.0*kc[:,0])/(0.5*ne*N),c='k')
plt.title('advection operator eigenvalues (imaginary component)')
plt.legend([r'$A$',r'$A_{PG;\Delta t}$',r'$-A_{PG;-\Delta t}^{\top}$',r'$\omega=k$'])
...
plt.plot((1.0*kc[:,0])/(0.5*ne*N),np.abs(wc)/(0.5*ne*N*2.0*np.pi),'go')
plt.plot((1.0*kd[:,0])/(0.5*ne*N),np.abs(wd)/(0.5*ne*N*2.0*np.pi),'bo')
plt.plot((1.0*ku[:,0])/(0.5*ne*N),np.abs(wu)/(0.5*ne*N*2.0*np.pi),'r+')
