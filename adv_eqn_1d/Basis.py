...
class LagrangeNode:
	def __init__(self,n,m):
		...
		self.M_ij_u = np.zeros((self.m+1,self.n+1),dtype=np.float64)
		self.M_ij_d = np.zeros((self.m+1,self.n+1),dtype=np.float64)
...
class LagrangeEdge:
	def __init__(self,n,m):
		...
		self.M_ij_u = np.zeros((m+1,n),dtype=np.float64)
		self.M_ij_d = np.zeros((m+1,n),dtype=np.float64)
