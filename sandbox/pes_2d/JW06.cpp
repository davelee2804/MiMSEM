...
int main(int argc, char** argv) {
    ...
    topo = new Topo(EL_ORD, N_ELSX);
    geom = new Geom(topo, NK, 2.0*M_PI*RAD_EARTH);
    ...
    rt    = new Vec[NK];
    exner = new Vec[NK];
    velz  = new Vec[topo->nElsX];
    for(ki = 0; ki < NK; ki++) {
        VecCreateSeq(MPI_COMM_SELF, topo->n1, &velx[ki] );
        VecCreateSeq(MPI_COMM_SELF, topo->n2, &rho[ki]  );
        VecCreateSeq(MPI_COMM_SELF, topo->n2, &exner[ki]);
        VecCreateSeq(MPI_COMM_SELF, topo->n2, &rt[ki]   );
    }
    for(ii = 0; ii < topo->nElsX; ii++) {
        VecCreateSeq(MPI_COMM_SELF, (NK-1)*topo->elOrd, &velz[ii]);
...
pe->init2(rho,theta_init);
sprintf(fieldname,"temp_test");
geom->write2(rho,fieldname,9999,true);

    pe->init1(velx,  u_init    );
    pe->init2(rho,   rho_init  );
    pe->init2(exner, exner_init);
    pe->init2(rt,    rt_init   );

    sprintf(fieldname,"velocity_h");
    geom->write1(velx,fieldname,0);
    sprintf(fieldname,"density");
    geom->write2(rho,fieldname,0,true);
    sprintf(fieldname,"exner");
    geom->write2(exner,fieldname,0,true);
    sprintf(fieldname,"rhoTheta");
    geom->write2(rt,fieldname,0,true);
    sprintf(fieldname,"velocity_z");
    geom->writeSerial(velz,fieldname,0,NK-1);
