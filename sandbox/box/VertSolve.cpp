...
void VertSolve::solve_coupled(L2Vecs* velz_i, L2Vecs* rho_i, L2Vecs* rt_i, L2Vecs* exner_i) {
    ...
    do {
        ...
        //if(max_norm_exner < 1.0e-8 && max_norm_w < 1.0e-7 && max_norm_rho < 1.0e-8 && max_norm_rt < 1.0e-8) done = true;
...
L2Vecs* F_rho_o, L2Vecs* F_rt_o, Vec* ul, WtQdUdz_mat* Rz)
{
    ...
    if(udwdx) uuz_j->CopyFromVert(udwdx->vz);
    ...
    do {
        ...
        for(int ii = 0; ii < topo->nElsX*topo->nElsX; ii++) {
            ...
            if(udwdx) {
                VecAXPY(F_w, 0.5*dt, udwdx->vz[ii]);
                VecAXPY(F_w, 0.5*dt, uuz_j->vz[ii]);
            }
#ifdef NEW_EOS
            vo->Assemble_EOS_Residual_new(ex, ey, rt_j->vz[ii], exner_j->vz[ii], F_exner);
#else
        ...
        if(udwdx) AssembleVertMomVort(ul, velz_j, ksp_x, M2, EtoF, Rz, uuz_j);
        ...
        //if(max_norm_exner < 1.0e-8 && max_norm_rho < 1.0e-8 && max_norm_rt < 1.0e-8) done = true;
        if(max_norm_exner < 1.0e-12 && max_norm_rho < 1.0e-12 && max_norm_rt < 1.0e-12) done = true;
        if(!rank) cout << itt << ":\t|d_exner|/|exner|: " << max_norm_exner << 
                                 "\t|d_w|/|w|: "          << max_norm_w     <<
                                 "\t|d_rho|/|rho|: "      << max_norm_rho   <<
...
void VertSolve::solve_schur_2(L2Vecs* velz_i, L2Vecs* rho_i, L2Vecs* rt_i, L2Vecs* exner_i, 
    ...
    do {
        ...
        if(udwdx) AssembleVertMomVort(u2l, velz_j, ksp_x, M2, EtoF, Rz, uuz_j);
...
void VertSolve::AssembleVertMomVort(Vec* ul, L2Vecs* velz, KSP ksp1, Mat M2, E21mat* EtoF, WtQdUdz_mat* Rz, L2Vecs* uuz) {
    int kk;
    ...
    for(kk = 0; kk < geom->nk-1; kk++) {
        ...
        MatMult(M2, velz->vh[kk], tmp);
        MatMult(EtoF->E12, tmp, tmp1);
        // KSP should be for a mass matrix with unit layer thickness !!!
