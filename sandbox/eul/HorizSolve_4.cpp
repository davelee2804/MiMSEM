...
HorizSolve::HorizSolve(Topo* _topo, Geom* _geom, double _dt) {
...
#endif
    N2_rt2_inv = new N_RT2_mat(topo, geom, edge);
    ...
    KSPSetTolerances(ksp1, 1.0e-14, 1.0e-50, PETSC_DEFAULT, 1000);
    ...
    KSPSetTolerances(ksp2, 1.0e-14, 1.0e-50, PETSC_DEFAULT, 1000);
                                ...
                                Vec velx1, Vec velx2, Vec rho1, Vec rho2, Vec fu, Vec _F, Vec _G, Vec uil, Vec ujl, Vec grad_pi)
{
    ...
    VecAXPY(fu, +1.0, utmp);
#ifdef RAYLEIGH
    if(level == geom->nk-1) VecAXPY(fu, +0.500*dt*RAYLEIGH, utmp);
    if(level == geom->nk-2) VecAXPY(fu, +0.250*dt*RAYLEIGH, utmp);
    if(level == geom->nk-3) VecAXPY(fu, +0.125*dt*RAYLEIGH, utmp);
#endif
...
void HorizSolve::assemble_biharmonic_temp(int lev, Vec rho, MatReuse reuse, Mat* BVISC) {
    ...
    MatScale(*BVISC, 0.5*dt*del2*del2);
}
                                   ...
                                   Vec F_u, Vec F_rho, Vec F_rt, Vec F_pi, Vec d_u, Vec d_rho, Vec d_rt, Vec d_pi, Vec dpil)
{
    ...
    coriolisMatInv(M_u, &M_u_inv, reuse);
...
void HorizSolve::assemble_and_update(int lev, Vec* theta, Vec velx_l, Vec velx_g, Vec rho, Vec rt, Vec pi, 
                                     Vec F_u, Vec F_rho, Vec F_rt, Vec F_pi, Vec dpil, Mat* OP, Schur* schur)
{
    MatReuse reuse = (!G_rt) ? MAT_INITIAL_MATRIX : MAT_REUSE_MATRIX;
...
/*
    VecCreateSeq(MPI_COMM_SELF, topo->n2, &tmp_h_l);
    MatMult(EtoF->E21, velx_g, tmp_h);
    VecScatterBegin(topo->gtol_2, tmp_h, tmp_h_l, INSERT_VALUES, SCATTER_FORWARD);
    VecScatterEnd(  topo->gtol_2, tmp_h, tmp_h_l, INSERT_VALUES, SCATTER_FORWARD);
    MatMatMult(T->M, M2inv->M, reuse, PETSC_DEFAULT, &M2theta_M2inv);
    T->assemble(tmp_h_l, lev, SCALE, true);
    MatMatMult(M2theta_M2inv, T->M, reuse, PETSC_DEFAULT, &M2theta_M2inv_M2du);
    MatScale(M2theta_M2inv_M2du, 0.5*dt);
*/

/*{
double norm_q, norm_m;
Vec _tmp_1, _tmp_2, _tmp_3, _tmp_4;
Mat _KT, _KT2;
VecCreateMPI(MPI_COMM_WORLD, topo->n2l, topo->nDofs2G, &_tmp_1);
VecCreateMPI(MPI_COMM_WORLD, topo->n2l, topo->nDofs2G, &_tmp_2);
VecCreateMPI(MPI_COMM_WORLD, topo->n1l, topo->nDofs1G, &_tmp_3);
VecCreateMPI(MPI_COMM_WORLD, topo->n1l, topo->nDofs1G, &_tmp_4);
VecSet(_tmp_1, 1.0);
VecSet(_tmp_3, 1.0);
MatMult(M2->M, _tmp_1, _tmp_2);
VecDot(_tmp_1, _tmp_2, &norm_m);
MatMult(Q_rt_rho, _tmp_1, _tmp_2);
VecDot(_tmp_1, _tmp_2, &norm_q);
if(!rank) cout << "|M|: " << norm_m << "\t|Q|: " << norm_q;

MatMult(M1_inv, _tmp_3, _tmp_4);
VecDot(_tmp_3, _tmp_4, &norm_m);
if(!rank) cout << "\t|M|^{-1}: " << norm_m;

MatMult(T->M, _tmp_1, _tmp_2);
VecDot(_tmp_1, _tmp_2, &norm_m);
if(!rank) cout << "\t|T|: " << norm_m;

MatTranspose(K->M, MAT_INITIAL_MATRIX, &_KT);
MatMatMult(K->M, _KT, MAT_INITIAL_MATRIX, PETSC_DEFAULT, &_KT2);
MatMult(_KT2, _tmp_1, _tmp_2);
VecDot(_tmp_1, _tmp_2, &norm_m);
norm_m=sqrt(norm_m);
if(!rank) cout << "\t|K|: " << norm_m;

VecNorm(theta_k, NORM_2, &norm_m);
VecNorm(velx_l, NORM_2, &norm_q);
if(!rank) cout << "\t|theta|: " << norm_m << "\t|velx|: " << norm_q << endl;
VecDestroy(&_tmp_1);
VecDestroy(&_tmp_2);
VecDestroy(&_tmp_3);
VecDestroy(&_tmp_4);
...
MatDestroy(&_KT2);
    ...
    MatAXPY(M_u, 1.0, R->M, DIFFERENT_NONZERO_PATTERN);
    coriolisMatInv(M_u, &M_u_inv, reuse);
    assemble_biharmonic_temp(lev, rho, reuse, OP); // mass matrix already included from vertical assembly
#else
    coriolisMatInv(R->M, &M_u_inv, reuse);
#endif
...
#ifdef IMP_VISC
    if(!schur) {
        MatAXPY(*OP, -1.0, L_rt_rt, DIFFERENT_NONZERO_PATTERN);
    }
#endif
    ...
    N2_rt->assemble(lev, SCALE, rt, pi);
#else
        N2_rt->assemble(rho, lev, SCALE, false);
        MatScale(N2_rt->M, -1.0*RD/CV);
#endif

        MatMatMult(L_rt_pi_N_pi_inv, N2_rt->M, reuse, PETSC_DEFAULT, &L_rt_pi_N_pi_inv_N_rho);
        //MatMatMult(L_rt_pi_N_pi_inv_N_rho, M2inv->M, reuse, PETSC_DEFAULT, &L_rt_pi_N_pi_inv_N_rho_M_inv);
        //schur->AddFromHorizMat(lev, L_rt_pi_N_pi_inv_N_rho_M_inv, schur->Q);
        schur->AddFromHorizMat(lev, L_rt_pi_N_pi_inv_N_rho, schur->D);

        MatMatMult(L_rho_pi_N_pi_inv, N2_rt->M, reuse, PETSC_DEFAULT, &L_rho_pi_N_pi_inv_N_rho);
        //schur->AddFromHorizMat(lev, L_rho_pi_N_pi_inv_N_rho, schur->P);

        //schur->AddFromHorizMat(lev, Q_rt_rho_M_rho_inv, schur->Q);
        schur->AddFromHorizMat(lev, Q_rt_rho, schur->D);
        schur->AddFromHorizMat(lev, L_rho_pi_N_pi_inv_N_rt, schur->G);
        MatScale(L_rt_rt, -1.0);
        schur->AddFromHorizMat(lev, L_rt_rt, schur->T);
        schur->AddFromHorizMat(lev, L_rt_pi_N_pi_inv_N_rt, schur->T);
    } else {
#ifdef IMP_VISC
    MatMatMult(Q_rt_rho_M_rho_inv, L_rho_pi_N_pi_inv_N_rt, MAT_INITIAL_MATRIX, PETSC_DEFAULT, &TEMP1);
    MatAYPX(TEMP1, -1.0, L_rt_pi_N_pi_inv_N_rt, DIFFERENT_NONZERO_PATTERN);
    MatAXPY(*OP, +1.0, TEMP1, DIFFERENT_NONZERO_PATTERN);
#else
    MatMatMult(Q_rt_rho_M_rho_inv, L_rho_pi_N_pi_inv_N_rt, MAT_INITIAL_MATRIX, PETSC_DEFAULT, OP);
    MatAYPX(*OP, -1.0, L_rt_pi_N_pi_inv_N_rt, DIFFERENT_NONZERO_PATTERN);
    MatAXPY(*OP, -1.0, L_rt_rt, DIFFERENT_NONZERO_PATTERN);
#endif
    }
                               ...
                               Vec F_u, Vec F_rho, Vec F_rt, Vec F_pi, Vec d_u, Vec d_rho, Vec d_rt, Vec d_pi, Vec dpil)
{
    ...
    Vec wl, wg, theta_k, ones, diag, tmp_h_1, tmp_h_2, tmp_u_1, tmp_u_2;
    PC pc;
    ...
    VecCreateMPI(MPI_COMM_WORLD, topo->n1l, topo->nDofs1G, &tmp_u_1);
    VecCreateMPI(MPI_COMM_WORLD, topo->n1l, topo->nDofs1G, &tmp_u_2);
    VecCreateMPI(MPI_COMM_WORLD, topo->n2l, topo->nDofs2G, &tmp_h_1);
    VecCreateMPI(MPI_COMM_WORLD, topo->n2l, topo->nDofs2G, &tmp_h_2);

    // back substitute (exner pressure done in the vertical update
    MatMult(L_rho_pi_N_pi_inv_N_rt, d_rt, tmp_h_1);
    MatMult(M2inv->M, tmp_h_1, tmp_h_2);
    VecAXPY(d_rho, -1.0, tmp_h_2); // F_rho term done in vertical update
...
void HorizSolve::update_delta_u(int lev, Vec* theta, Vec velx_l, Vec velx_g, Vec rho, Vec rt, Vec pi, 
                                Vec F_u, Vec d_u, Vec d_rho, Vec d_rt, Vec d_pi, Vec dpil)
{
    MatReuse reuse = MAT_REUSE_MATRIX;
    Vec wl, wg, theta_k, ones, diag, tmp_u_1, tmp_u_2;
    PC pc;
...
void HorizSolve::assemble_and_update_2(int lev, Vec velx_l, Vec velx_g, Vec rho, Vec rt, 
                                       Vec F_u, Vec F_rho, Vec dpil)
{
    MatReuse reuse = (!G_rho) ? MAT_INITIAL_MATRIX : MAT_REUSE_MATRIX;
    Vec wl, wg, ones, diag, tmp_h;
    ...
    K->assemble(dpil, lev, SCALE); // 0.5 factor included here
    MatTranspose(K->M, reuse, &KT);
    N2_rt2_inv->assemble(lev, SCALE, rho);
    MatMatMult(KT, N2_rt2_inv->M, reuse, PETSC_DEFAULT, &G_pi_rt2_inv);
    MatMatMult(G_pi_rt2_inv, T->M, reuse, PETSC_DEFAULT, &G_rho);
    MatScale(G_rho, -dt);

    // D_rho
    MatMatMult(M2->M, EtoF->E21, MAT_REUSE_MATRIX, PETSC_DEFAULT, &M2D);
    MatMatMult(M2D, M1_inv, MAT_REUSE_MATRIX, PETSC_DEFAULT, &M2DM1_inv);
    F->assemble(rho, lev, true, SCALE);
    MatMatMult(M2DM1_inv, F->M, MAT_REUSE_MATRIX, PETSC_DEFAULT, &D_rho);
    MatScale(D_rho, 0.5*dt);
    ...
    m0->assemble(lev, SCALE);
    MatMatMult(M1_inv, EtoF->E12, MAT_REUSE_MATRIX, PETSC_DEFAULT, &M1invDT);
    assemble_biharmonic(lev, MAT_REUSE_MATRIX, &M_u);
    MatAXPY(M_u, 1.0, R->M, DIFFERENT_NONZERO_PATTERN);
    coriolisMatInv(M_u, &M_u_inv, MAT_REUSE_MATRIX);
#else
    coriolisMatInv(R->M, &M_u_inv, MAT_REUSE_MATRIX);
#endif

    // assemble the secondary operators
    MatMatMult(D_rho, M_u_inv, MAT_REUSE_MATRIX, PETSC_DEFAULT, &D_rho_M_u_inv);
    MatMatMult(D_rho_M_u_inv, G_rho, reuse, PETSC_DEFAULT, &L_rho_rho); // mass matrix added from the vertical assembly
    MatScale(L_rho_rho, -1.0);
...
void HorizSolve::update_delta_u_2(int lev, Vec velx_l, Vec velx_g, Vec rho, Vec rt, 
                                       Vec F_u, Vec F_rho, Vec dpil, Vec d_u, Vec d_rho)
{
    MatReuse reuse = MAT_REUSE_MATRIX;
    Vec wl, wg, ones, diag, tmp_u;
    PC pc;
...
//#endif
    ...
    MatMult(G_rho, d_rho, tmp_u);
    VecAXPY(F_u, 1.0, tmp_u);
    VecScale(F_u, -1.0);
    KSPSolve(ksp_u, F_u, d_u);
...
void HorizSolve::assemble_M_rho(int lev, Vec velx_l, Schur* schur) {
    Vec diag, ones;
    ...
    MatMatMult(M1_inv, EtoF->E12, MAT_REUSE_MATRIX, PETSC_DEFAULT, &M1invDT);
    MatMatMult(M1invDT, M2->M, MAT_REUSE_MATRIX, PETSC_DEFAULT, &M1invDTM2);
    MatMatMult(K->M, M1invDTM2, MAT_INITIAL_MATRIX, PETSC_DEFAULT, &UdotGRAD);
    MatScale(UdotGRAD, dt);
    MatAssemblyBegin(UdotGRAD, MAT_FINAL_ASSEMBLY);
    MatAssemblyEnd(  UdotGRAD, MAT_FINAL_ASSEMBLY);

    schur->AddFromHorizMat(lev, UdotGRAD, schur->P);

    MatDestroy(&UdotGRAD);
}

void HorizSolve::assemble_and_update_3(int lev, Vec* theta, Vec velx_l, Vec velx_g, Vec rho, Vec rt, Vec pi, 
                                       Vec F_u, Vec F_rho, Vec F_rt, Vec F_pi, Vec dpil, Vec Du, Schur* schur, int itt)
{
    ...
    //MatMatMult(M2DM1_inv, F->M, reuse, PETSC_DEFAULT, &D_rt);
    MatScale(D_rt, 0.5*dt);
    ...
    assemble_biharmonic_temp(lev, rho, reuse, &_PCx);
    schur->AddFromHorizMat(lev, _PCx, schur->VISC);
    MatDestroy(&_PCx);

    // u.GRAD
    K->assemble(velx_l, lev, SCALE); // 0.5 factor included here
    MatMatMult(K->M, M1invDTM2, reuse, PETSC_DEFAULT, &UdotGRAD);
    MatScale(UdotGRAD, dt);
...
//#endif
    ...
    MatMatMult(G_pi, N2_pi_inv->M, reuse, PETSC_DEFAULT, &G_pi_N_pi_inv);
    MatMatMult(G_pi_N_pi_inv,  N2_rt->M, MAT_INITIAL_MATRIX, PETSC_DEFAULT, &G_pi_N_pi_inv_N_rt);
    MatAYPX(G_pi_N_pi_inv_N_rt, -1.0, G_rt, DIFFERENT_NONZERO_PATTERN);
    
    // add to the global 3D matrices
    MatMatMult(D_rho, M_u_inv, reuse, PETSC_DEFAULT, &D_rho_M_u_inv);
    MatMatMult(D_rt,  M_u_inv, reuse, PETSC_DEFAULT, &D_rt_M_u_inv );
    ...
    schur->AddFromHorizMat(lev, L_rho_rt, schur->L_rho);
    schur->AddFromHorizMat(lev, L_rt_rt,  schur->L_rt );
    schur->AddFromHorizMat(lev, Q_rt_rho_M_rho_inv, schur->Q);
    schur->AddFromHorizMat(lev, Q_rt_rho, schur->Q2);
    schur->AddFromHorizMat(lev, UdotGRAD, schur->A);

    // update the residuals
    MatMult(G_pi_N_pi_inv, F_pi, ones);
    VecAXPY(F_u, -1.0, ones);

    MatMult(D_rt_M_u_inv, F_u, tmp_h);
    VecAXPY(F_rt, -1.0, tmp_h);

    MatMult(D_rho_M_u_inv, F_u, Du);
...
void HorizSolve::update_delta_u_3(int lev, Vec* theta, Vec velx_l, Vec velx_g, Vec rho, Vec rt, Vec pi, 
                                  Vec F_u, Vec F_rho, Vec d_u, Vec d_rho, Vec d_rt, Vec d_pi, Vec dpil)
{
    ...
    //MatMatMult(KT, M2_rho_inv->M, reuse, PETSC_DEFAULT, &KTM2_inv);
    //MatMatMult(KTM2_inv, M2->M, reuse, PETSC_DEFAULT, &G_rt);
    MatMatMult(KT, M2_rho_inv->M, MAT_REUSE_MATRIX, PETSC_DEFAULT, &KTM2_inv);
    MatMatMult(KTM2_inv, M2->M, MAT_REUSE_MATRIX, PETSC_DEFAULT, &G_rt);
    MatScale(G_rt, dt);
    ...
    MatMatMult(M1_inv, EtoF->E12, MAT_REUSE_MATRIX, PETSC_DEFAULT, &M1invDT);
    MatMatMult(M1invDT, M2->M, MAT_REUSE_MATRIX, PETSC_DEFAULT, &M1invDTM2);
    MatMatMult(F->M, M1invDTM2, MAT_REUSE_MATRIX, PETSC_DEFAULT, &G_pi);
    MatScale(G_pi, 0.5*dt);
    ...
    K->assemble(velx_l, lev, SCALE); // 0.5 factor included here
    MatMatMult(EtoF->E12, K->M, MAT_INITIAL_MATRIX, PETSC_DEFAULT, &DK);
    MatAXPY(M_u, dt, DK, DIFFERENT_NONZERO_PATTERN);
    MatDestroy(&DK);
    ...
    MatMult(G_pi_N_pi_inv_N_rt, d_rt, ones);
    VecAXPY(F_u, 1.0, ones);
    VecScale(F_u, -1.0);
