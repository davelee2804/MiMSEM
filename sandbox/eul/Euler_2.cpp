...
void Euler::horizMomRHS(Vec uh, Vec* theta_l, Vec exner, int lev, Vec Fu, Vec Flux, Vec uzb, Vec uzt, Vec velz_b, Vec velz_t) {
    double dot;
    Vec wl, wi, Ru, d2u, d4u, theta_k, dExner, dp;
...
void Euler::HorizRHS(Vec* velx, L2Vecs* rho, L2Vecs* rt, Vec* exner, Vec* Fu, Vec* Fp, Vec* Ft, Vec* velz) {
    ...
    if(Fp) massRHS(velx, rho->vl, Fp, Flux);
    if(Ft) tempRHS(Flux, theta->vl, Ft, rho->vl, exner);
...
void Euler::Trapazoidal(Vec* velx, Vec* velz, Vec* rho, Vec* rt, Vec* exner, bool save) {
    ...
    velz_0->CopyFromVert(velz);    velz_0->VertToHoriz();  velz_0->UpdateGlobal();
    ...
    rho_1->UpdateLocal(); rho_1->HorizToVert();
    rt_1->UpdateLocal();  rt_1->HorizToVert();
 ...
 vert->solve_schur_2(velz_h, rho_h, rt_h, exner_h, uuz, velx_0, velx_2, NULL, NULL);
    ...
    //vert->solve_schur(velz_h, rho_h, rt_h, exner_h, uuz, del2, M1, M2, EtoF, ksp1, F_rho_h, F_rt_h);
 vert->solve_schur_2(velz_h, rho_h, rt_h, exner_h, uuz, velx_0, velx, NULL, NULL);
...
if(firstStep) for(int kk = 0; kk < geom->nk-1; kk++) VecCopy(uzl[kk], uzl_prev[kk]);
    for(int kk = 0; kk < geom->nk; kk++) {
        vert->horiz->momentum_rhs(kk, theta_0->vl, uzl, uzl, velz_0->vh, velz_0->vh, exner_0->vh[kk],
...
M1->assemble(kk, SCALE, true);
    ...
    HorizVort(velx);
    for(int kk = 0; kk < geom->nk; kk++) {
        vert->horiz->momentum_rhs(kk, vert->theta_h->vl, uzl, uzl_prev, velz_h->vh, velz_0->vh, vert->exner_h->vh[kk],
                                  velx_0[kk], velx[kk], ul[kk], ul_prev[kk], Fu_1[kk]);

M1->assemble(kk, SCALE, true);
        MatMult(M1->M, velx_0[kk], bu);
        VecAXPY(bu, -dt, Fu_1[kk]);
        KSPSolve(ksp1, bu, velx[kk]);
