...
void Euler::solve_gs(Vec* velx, L2Vecs* velz_i, L2Vecs* rho_i, L2Vecs* rt_i, L2Vecs* exner_i, bool save) {
    bool done = false;
    ...
    do {
...
#ifdef ADV_EOS
            vo->AssembleConstWithRho(ex, ey, exner_i->vz[ii], vo->VB);
            MatMult(vo->VB, dG_z, F_exner->vz[ii]);
            VecCopy(exner_j->vz[ii], vert->_tmpB1);
            VecAXPY(vert->_tmpB1, -1.0, exner_i->vz[ii]);
            vo->AssembleConstWithRho(ex, ey, rt_i->vz[ii], vo->VB);
            MatMult(vo->VB, vert->_tmpB1, vert->_tmpB2);
            VecAYPX(F_exner->vz[ii], dt*RD/CV, vert->_tmpB2);
#endif
        ...
        for(int lev = 0; lev < geom->nk; lev++) {
            ...
            horiz->assemble_residual(lev, theta_h->vl, dudz_i->vh, dudz_j->vh, velz_i->vh, velz_j->vh, exner_h->vh[lev], 
                                velx_i->vh[lev], velx_j->vh[lev], rho_i->vh[lev], rho_j->vh[lev], F_u->vh[lev], _F, _G,
                                velx_i->vl[lev], velx_j->vl[lev], velx_h->vl[lev]);
...
#ifdef ADV_EOS
            horiz->T->assemble(exner_i->vl[lev], lev, SCALE, true);
            MatMult(horiz->T->M, dG, h_tmp);
            VecAXPY(F_exner->vh[lev], dt*RD/CV, h_tmp);
#endif
...
#ifdef ADV_EOS
        F_exner->UpdateLocal(); F_exner->HorizToVert();
#endif
        ...
        for(int ii = 0; ii < topo->nElsX*topo->nElsX; ii++) {
            ...
            vert->update_residuals(ex, ey, theta_h->vz[ii], velz_j->vz[ii], rho_h->vz[ii], rt_h->vz[ii], exner_h->vz[ii], 
        ...
        for(int lev = 0; lev < geom->nk; lev++) {
            horiz->assemble_and_update(lev, theta_h->vl, velx_h->vh[lev], rho_h->vl[lev], rt_h->vl[lev], exner_h->vl[lev],
                                       F_u->vh[lev], F_rho->vh[lev], F_rt->vh[lev], F_exner->vh[lev], false, false, NULL, NULL, velx_h->vl[lev]); // eos update done with vert residual update
        ...
        for(int ii = 0; ii < topo->nElsX*topo->nElsX; ii++) {
            ex = ii%topo->nElsX;
            ey = ii/topo->nElsX;
            vert->assemble_pc(ex, ey, theta_h->vz[ii], velz_j->vz[ii], rho_h->vz[ii], rt_h->vz[ii], exner_h->vz[ii], true);

            VecCopy(F_rt->vz[ii], dF_z);
            VecAXPY(dF_z, 1.0, lap_d_exner->vz[ii]);
            KSPSolve(vert->ksp_pi, dF_z, d_exner->vz[ii]);
        }
        d_exner->VertToHoriz(); d_exner->UpdateGlobal();

        // back substitution
        for(int lev = 0; lev < geom->nk; lev++) {
            horiz->set_deltas(lev, theta_h->vl, velx_h->vh[lev], rho_h->vl[lev], rt_h->vl[lev], exner_h->vl[lev], 
                       F_u->vh[lev], F_rho->vh[lev], F_exner->vh[lev], d_u->vh[lev], d_rho->vh[lev], d_rt->vh[lev], d_exner->vh[lev], 
                       false, false, NULL, NULL, velx_h->vl[lev]);
        }
        F_rho->UpdateLocal(); F_rho->HorizToVert();
