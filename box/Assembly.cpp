...
Umat::Umat(Topo* _topo, Geom* _geom, LagrangeNode* _l, LagrangeEdge* _e) {
    ...
    MatCreate(MPI_COMM_WORLD, &Mo);
    MatSetSizes(Mo, topo->n1l, topo->n1l, topo->nDofs1G, topo->nDofs1G);
    MatSetType(Mo, MATMPIAIJ);
    MatMPIAIJSetPreallocation(Mo, 8*U->nDofsJ, PETSC_NULL, 8*U->nDofsJ, PETSC_NULL);

    assemble(0, SCALE, true, M);
    assemble(0, SCALE, false, Mo);

    delete U;
}

void Umat::assemble(int lev, double scale, bool vert_scale, Mat _M) {
...
Wmat::Wmat(Topo* _topo, Geom* _geom, LagrangeEdge* _e) {
    ...
    MatCreate(MPI_COMM_WORLD, &Mo);
    MatSetSizes(Mo, topo->n2l, topo->n2l, topo->nDofs2G, topo->nDofs2G);
    MatSetType(Mo, MATMPIAIJ);
    MatMPIAIJSetPreallocation(Mo, 4*W->nDofsJ, PETSC_NULL, 2*W->nDofsJ, PETSC_NULL);
...
void Wmat::assemble(int lev, double scale, bool vert_scale, Mat _M) {
    ...
    for(ey = 0; ey < topo->nElsX; ey++) {
        for(ex = 0; ex < topo->nElsX; ex++) {
            ...
            for(ii = 0; ii < mp12; ii++) {
                ...
                Qaa[ii][ii] = hi*J[0][0]*J[0][0]*Q->A[ii][ii]*(scale/det);
                Qbb[ii][ii] = hi*J[1][1]*J[1][1]*Q->A[ii][ii]*(scale/det);
...
Pvec::Pvec(Topo* _topo, Geom* _geom, LagrangeNode* _l) {
    ...
    VecCreateMPI(MPI_COMM_WORLD, topo->n0l, topo->nDofs0G, &vg1);
...
void Pvec::assemble(int lev, double scale, Vec _vg) {
    ...
    VecScatterBegin(topo->gtol_0, vl, _vg, ADD_VALUES, SCATTER_REVERSE);
    VecScatterEnd(topo->gtol_0, vl, _vg, ADD_VALUES, SCATTER_REVERSE);

    // and back to local vector
    VecScatterBegin(topo->gtol_0, _vg, vl, INSERT_VALUES, SCATTER_FORWARD);
    VecScatterEnd(topo->gtol_0, _vg, vl, INSERT_VALUES, SCATTER_FORWARD);
}
    ...
    for(ey = 0; ey < topo->nElsX; ey++) {
        for(ex = 0; ex < topo->nElsX; ex++) {
            ...
            for(ii = 0; ii < mp12; ii++) {
                ...
                Qaa[ii][ii] = 0.5*ux[0]*J[0][0]*Q->A[ii][ii]*(scale/det);
                Qab[ii][ii] = 0.5*ux[1]*J[1][1]*Q->A[ii][ii]*(scale/det);
    ...
    for(ey = 0; ey < topo->nElsX; ey++) {
        for(ex = 0; ex < topo->nElsX; ex++) {
            ...
            for(ii = 0; ii < mp12; ii++) {
                ...
                Qab[ii][ii] = vort*(-J[0][0]*J[1][1])*Q->A[ii][ii]*(scale/det);
                Qba[ii][ii] = vort*(+J[0][0]*J[1][1])*Q->A[ii][ii]*(scale/det);
    ...
    for(ey = 0; ey < topo->nElsX; ey++) {
        for(ex = 0; ex < topo->nElsX; ex++) {
            ...
            for(ii = 0; ii < mp12; ii++) {
                ...
                Qaa[ii][ii] = ux[0]*J[0][0]*Q->A[ii][ii]*(scale/det);
                Qba[ii][ii] = ux[1]*J[1][1]*Q->A[ii][ii]*(scale/det);
            }
...
//                Qaa[ii][ii] = (+ux[1]*J[1][0] - ux[0]*J[0][0])*Q->A[ii][ii]*(scale/det);
                ...
                Qaa[ii][ii] = ux[0]*J[0][0]*Q->A[ii][ii]*(scale/det);
                Qab[ii][ii] = ux[1]*J[1][1]*Q->A[ii][ii]*(scale/det);
            }
...
void Uvec::assemble(int lev, double scale, bool vert_scale, Vec vel) {
    ...
    double _u[2], Qaa[99], Qbb[99], rhs[99];
    ...
    for(ey = 0; ey < topo->nElsX; ey++) {
        for(ex = 0; ex < topo->nElsX; ex++) {
            ...
            for(ii = 0; ii < mp12; ii++) {
                ...
                Qaa[ii] = J[0][0]*J[0][0]*Q->A[ii][ii]*(scale/det);
                Qbb[ii] = J[1][1]*J[1][1]*Q->A[ii][ii]*(scale/det);
...
void Uvec::assemble_hu(int lev, double scale, bool vert_scale, Vec vel, Vec rho) {
    ...
    double _u[2], _r, Qaa[99], Qbb[99], rhs[99];
    ...
    for(ey = 0; ey < topo->nElsX; ey++) {
        for(ex = 0; ex < topo->nElsX; ex++) {
            ...
            for(ii = 0; ii < mp12; ii++) {
                ...
                Qaa[ii] = 0.5*_uxg[0]*J[0][0]*Q->A[ii][ii]*(scale/det);
                Qab[ii] = 0.5*_uxg[1]*J[1][1]*Q->A[ii][ii]*(scale/det);
