...
void Phmat::assemble_up(Vec ul, Vec hl, double dt) {
    ...
    double ux[2], lx[99], ly[99], det, **J, ux2[2], hx, uxl[2], bl[2], tau, tmp;
    ...
    double** PtQP = Alloc2D(P->nDofsJ, P->nDofsJ);
    double* PtQPflat = new double[P->nDofsJ*P->nDofsJ];
    double* ones = new double[topo->n1];
    ...
    for(ii = 0; ii < topo->n1; ii++) ones[ii] = 1.0;
    ...
    for(ey = 0; ey < topo->nElsX; ey++) {
        for(ex = 0; ex < topo->nElsX; ex++) {
            ...
            for(ii = 0; ii < mp12; ii++) {
                ...
                //geom->interp1_l(ex, ey, ii%mp1, ii/mp1, uArray, uxl);
                //geom->interp1_l(ex, ey, ii%mp1, ii/mp1, ones, bl);
                //tmp = uxl[0]*bl[0] + uxl[1]*bl[1];
                //tau = 1.0/(1.0/dt + sqrt(tmp*tmp));
...
void RotMat_up::assemble(Vec q0, Vec ul, double dt) {
    ...
    double det, **J, vort, ux[2], lx[99], ly[99], ux2[2], tau, uxl[2], bl[2], tmp, tmp2, tau_max = -1.0;
    ...
    for(ey = 0; ey < topo->nElsX; ey++) {
        for(ex = 0; ex < topo->nElsX; ex++) {
            ...
            for(ii = 0; ii < mp12; ii++) {
                ...
                tmp = sqrt(ux[0]*ux[0] + ux[1]*ux[1]);
                tmp *= bl[0]/sqrt(det);
                tmp2 = bl[0]*0.5*dt*(ux[0]*ux[0] + ux[1]*ux[1])/det;
                tau = 1.0/(1.0/dt + tmp /*+ tmp2*/);
...
void RotMat_up::assemble_supg(Vec q0, Vec ul, Vec dql, double tau, double dt, Vec qi) {
    int ex, ey, ei, ii, mp1, mp12;
    int *inds_x, *inds_y;
    double det, **J, vort, ux[2], dq[2], vort_i, uxl[2], bl[2], tmp, tau2;
    ...
    for(ey = 0; ey < topo->nElsX; ey++) {
        for(ex = 0; ex < topo->nElsX; ex++) {
            ...
            for(ii = 0; ii < mp12; ii++) {
                ...
                tmp = sqrt(ux[0]*ux[0] + ux[1]*ux[1]);
                tmp *= bl[0]/sqrt(det);
                tau2 = 1.0/(1.0/tau + tmp);

                vort -= tau2*(ux[0]*dq[1] - ux[1]*dq[0] + (vort - vort_i)/dt);
