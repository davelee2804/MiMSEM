...
double w_init(double* x) {
    ...
    return (2.0*W2_U0/RAD_SPHERE + 2.0*W2_OMEGA)*(-cos(lambda)*cos(theta)*sin(0.25*M_PI) + sin(theta)*cos(0.25*M_PI));
}
...
double u_init(double* x) {
    ...
    return W2_U0*(cos(theta)*cos(0.25*M_PI) + cos(lambda)*sin(theta)*sin(0.25*M_PI));
}
...
double v_init(double* x) {
    ...
    return -W2_U0*sin(lambda)*sin(0.25*M_PI);
}
...
double h_init(double* x) {
    ...
    double b = -cos(lambda)*cos(theta)*sin(0.25*M_PI) + sin(theta)*cos(0.25*M_PI);
...
int main(int argc, char** argv) {
    ...
    //int nSteps = 1*1200;
    //int dumpEvery = 1*240;
    double dt = 0.5*240.0; // time step for 2 3rd order elements per dimension per face
    int nSteps = 2*1800;
    int dumpEvery = 2*15;
    ofstream file;
    Topo* topo;
