...
class Proc:
	...
	def writeLocalSizes(self):
		...
		a[2] = self.n1yl
		a[3] = self.n2l
		np.savetxt('../run/input/local_sizes_%.4u'%self.procID + '.txt', a, fmt='%u')
...
class ParaCube:
	...
	def print_nodes(self,pi):
		...
		np.savetxt('../run/input/nodes_%.4u'%pi + '.txt', proc.loc0, fmt='%u')

	def print_edges(self,pi,dim):
		proc = self.procs[pi]
		if dim == 0:
			#print str(pi) + ' (x):\t' + str(proc.loc1x)
			np.savetxt('../run/input/edges_x_%.4u'%pi + '.txt', proc.loc1x, fmt='%u')
		else:
			#print str(pi) + ' (y):\t' + str(proc.loc1y)
			np.savetxt('../run/input/edges_y_%.4u'%pi + '.txt', proc.loc1y, fmt='%u')

	def print_faces(self,pi):
		proc = self.procs[pi]
		#print str(pi) + ':\t' + str(proc.loc2)
		np.savetxt('../run/input/faces_%.4u'%pi + '.txt', proc.loc2, fmt='%u')
