    ...
    for(ey = 0; ey < topo->nElsX; ey++) {
        for(ex = 0; ex < topo->nElsX; ex++) {
            ...
            for(ii = 0; ii < np12; ii++) {
                ...
                entries[ii] *= 2.0/geom->thick[lev][inds_g[ii]];
            }
...
UtQWmat::UtQWmat(Topo* _topo, Geom* _geom, LagrangeNode* _l, LagrangeEdge* _e) {
    ...
    Qab = Alloc2D(Q->nDofsI, Q->nDofsJ);
    UtQaa = Alloc2D(U->nDofsJ, Q->nDofsJ);
    VtQab = Alloc2D(U->nDofsJ, Q->nDofsJ);
...
void UtQWmat::assemble(int lev) {
    ...
    for(ey = 0; ey < topo->nElsX; ey++) {
        for(ex = 0; ex < topo->nElsX; ex++) {
            ...
            Mult_IP(V->nDofsJ, Q->nDofsJ, Q->nDofsI, Vt, Qab, VtQab);

            Mult_IP(U->nDofsJ, W->nDofsJ, W->nDofsI, UtQaa, W->A, UtQW);
            Mult_IP(V->nDofsJ, W->nDofsJ, W->nDofsI, VtQab, W->A, VtQW);
...
UtQWmat::~UtQWmat() {
    ...
    Free2D(U->nDofsJ, VtQW);
    Free2D(U->nDofsJ, UtQaa);
    Free2D(U->nDofsJ, VtQab);
    delete[] UtQWflat;
    delete[] VtQWflat;
    delete U;
    delete V;
