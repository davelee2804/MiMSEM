...
*/
void PrimEqns::progExner(Vec rt_i, Vec DivG, Vec exner_i, Vec* exner_f, int lev) {
    double scale = 1.0e8;
    Vec rt_l, rhs, dG_l;
    PC pc;
    KSP kspE;
...
/*
    ...
    VecScatterEnd(topo->gtol_2, dG, dG_l, INSERT_VALUES, SCATTER_FORWARD);
    VecGetArray(dG_l, &dgArray);
    for(ey = 0; ey < topo->nElsX; ey++) {
        for(ex = 0; ex < topo->nElsX; ex++) {
            ei = ey*topo->nElsX + ex;
            inds2 = topo->elInds2_l(ex, ey);
            VecGetArray(Gv[ei], &gvArray);
            for(jj = 0; jj < n2; jj++) {
                if(lev > 0) {
                    dgArray[inds2[jj]] += gvArray[(lev-1)*n2+jj];
                }
                if(lev < geom->nk-1) {
                    dgArray[inds2[jj]] -= gvArray[(lev+0)*n2+jj];
                }
            }
            VecRestoreArray(Gv[ei], &gvArray);
        }
    }
...
void PrimEqns::SolveRK2(Vec* velx, Vec* velz, Vec* rho, Vec* rt, Vec* exner, bool save) {
    ...
    for(kk = 0; kk < geom->nk; kk++) {
        ...
        // exner pressure
        progExner(rt_i[kk], Ft1[kk], exner[kk], &exner_h[kk], kk);
    }
    ...
    for(kk = 0; kk < geom->nk; kk++) {
        ...
        progExner(rt_i[kk], Ft1[kk], exner_i[kk], &exner_f, kk);
...
void PrimEqns::SolveEuler(Vec* velx, Vec* velz, Vec* rho, Vec* rt, Vec* exner, bool save) {
    ...
    for(kk = 0; kk < geom->nk; kk++) {
        ...
        progExner(rt_i[kk], Ft1[kk], exner[kk], &exner_f, kk);
        VecCopy(exner_f, exner[kk]);
        VecDestroy(&exner_f);
#endif
    }
