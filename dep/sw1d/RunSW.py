...
for step in np.arange(n_steps)+1:
	...
	en = sw.energy(u1,h1)
	e1[step] = (e0-en)/e0
	sw.solve_imex_2(u2,h2)
	en = sw.energy(u2,h2)
	e2[step] = (e0-en)/e0
	#sw.solve_rk2(u3,h3)
	#en = sw.energy(u3,h3)
	#e3[step] = (e0-en)/e0
	print('energy conservation: ' + str(e1[step]), '\t', str(e2[step]), '\t', str(e3[step]))
	if step%20==0:
		ux = plot_u(u1,X,topo,topo_q,dX)
		plt.plot(X,ux)
		ux = plot_u(u2,X,topo,topo_q,dX)
		plt.plot(X,ux,'-.')
		#ux = plot_u(u3,X,topo,topo_q,dX)
		#plt.plot(X,ux,'.')
...
#plt.semilogy(dt*np.arange(n_steps+1),np.abs(e3))
plt.legend(['energy conserving newton','energetically balanced imex'])
#plt.legend(['energy conserving newton','energetically balanced imex','runge-kutta-2'])
